...
from main import Class, ssic_alpha_index_filepath, ssic_detailed_def_filepath
...
st.header(f'🏷️ Class, {Class} Categories', divider='rainbow')
...
with col2:
    ...
    if section_filter:
        filtered_df_ref = ssic_sl[ssic_sl['Class'].str.contains(section_filter, case=False)]
    else:
        filtered_df_ref = ssic_sl

    if ssic_2020_title_filter:
        filtered_df_ref = filtered_df_ref[filtered_df_ref['Class Title'].str.contains(ssic_2020_title_filter, case=False)]
    else:
